...
function App() {
  ...
  const fetchEmails = async () => {
    try {
      const res = await fetch('http://localhost:8000/api/emails', { credentials: 'include' })
      if (res.ok) {
  ...
  const handleSync = async () => {
    setSyncing(true)
    try {
      const res = await fetch('http://localhost:8000/api/sync', { method: 'POST', credentials: 'include' })
